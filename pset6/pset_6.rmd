...
knitr::opts_knit$set(root.dir = "~/Documents/UCHICAGO/Fourth Year/Winter23/ECMA/casual/pset6")
...
dt_psid[ , `:=` (agesq = age^2, edusq = edu^2, re74sq = re74^2, re75sq = re75^2, u74black=u74*black)]
...
```{r}
trimmed_df <- subset(dt_psid, logit_propensity >= max(p_cl, p_tl) & logit_propensity <= min(p_cu, p_tu))
```

```{r}
length(control$logit_propensity) - length((trimmed_df %>% filter(treat == 0))$logit_propensity)
```
```{r}
length(treatment$logit_propensity) - length((trimmed_df %>% filter(treat == 1))$logit_propensity)
```
...
trimmed_df <- dplyr::mutate(trimmed_df, stratum = .bincode(trimmed_df$logit_propensity, b))
...
for(stratum_i in 1: 10){
  ...
  vars = setdiff(colnames(subset), rmcoll(subset))
  
  sur_system <- purrr::map(vars, function(x) stats::formula(paste(x, "~ treat")))
  null_system <- purrr::map(vars, function(x) stats::formula(paste(x, "~ 1")))
  
  sur_fit <- systemfit::systemfit(sur_system, data=trimmed_df, method="SUR")
  null_fit <- systemfit::systemfit(null_system, data=trimmed_df, method="SUR")
  
  even <- function(x) x %% 2 == 0
  tab_sur <- round(summary(sur_fit)$coefficients[even(1:dim(summary(sur_fit)$coefficients)[1]),
                                                 c("Estimate", "Std. Error", "Pr(>|t|)")], 3)
  colnames(tab_sur)[colnames(tab_sur) =="Pr(>|t|)"] <- "P_value"
  colnames(tab_sur)[colnames(tab_sur) =="Std. Error"] <- "Std_Error"
  tab_sur <- cbind(rownames(tab_sur), data.frame(tab_sur, row.names = NULL))
  colnames(tab_sur)[1] <- 'DepVar' 
  tab_sur$DepVar <- gsub('_treat', "", tab_sur$DepVar)
  
  knitr::kable(tab_sur, format = 'simple')
}
