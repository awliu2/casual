...
```{r}
formula = as.formula(earns ~ dit + factor(individual) + factor(dyear2) - 1)
ols <- lm(formula, data = long_psid)
```

```{r}
ols$coefficients['dit']
```

```{r}
formula2 = as.formula(earns ~ dit + factor(individual) + factor(dyear2))
ols2 <- lm(formula2, data = long_psid)
```

```{r}
ols2$coefficients['dit']
```
