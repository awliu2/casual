...
\NormalTok{p }\OtherTok{\textless{}{-}}\NormalTok{ ggplot2}\SpecialCharTok{::}\FunctionTok{ggplot}\NormalTok{(plot\_df, ggplot2}\SpecialCharTok{::}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pscore, }\AttributeTok{y =}\NormalTok{ re78)) }\SpecialCharTok{+}
...
\CommentTok{\# Save plot object to PDF file}
\NormalTok{ggplot2}\SpecialCharTok{::}\FunctionTok{ggsave}\NormalTok{(filename\_plot\_2, }\AttributeTok{plot =}\NormalTok{ p)}
\end{Highlighting}
\end{Shaded}
  \item
...
\item
  ...
  \item
    (25 p) Test that the average of each pretreatment variable is the
    same for treated and control units within each stratum: the
    pre-treatment variables are
