...
```{r}
formula = as.formula(earns ~ treat + factor(individual) + factor(dyear2) - 1)
ols <- lm(formula, data = long_psid)
```

```{r}
ols$coefficients['treat']
```

In our linear regression we drop the constant due to the dummy variable trap and successfully estimate $\widehat{ATT}_{78}^{LSDV} = 5657.098$.
