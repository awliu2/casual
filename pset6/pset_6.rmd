...
```{r}
trimmed <- subset(dt_psid, logit_propensity >= max(p_cl, p_tl) & logit_propensity <= min(p_cu, p_tu))
```

```{r}
length(control$logit_propensity) - length((trimmed %>% filter(treat == 0))$logit_propensity)
```
```{r}
length(treatment$logit_propensity) - length((trimmed %>% filter(treat == 1))$logit_propensity)
```
